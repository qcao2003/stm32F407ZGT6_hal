...
uint8_t NRF24L01_Check(void)
{
	...
	state = NRF24L01_Write_Buf(NRF_WRITE_REG+TX_ADDR,buf,5);//写入5个字节的地址.  
	printf("Write state is 0x%X\r\n",state);
	state = NRF24L01_Read_Buf(TX_ADDR,buf,5); //读出写入的地址 
	printf("Read state is 0x%X\r\n",state);
